...
5. **Query Data:** Run `python tools/coach_context.py` first, using a targeted `--profile`, `--section`, or `--flags` when possible; only then interrogate `src/db/user_data.db` for exact row-level follow-up metrics. Query `coach_decisions` for active/recent decisions relevant to the same topic, date range, workout, or context before making a new coaching call.
...
- `coach/coach_instructions.md`: Stable coaching policy, database schema, query patterns, and operational rules. Only change on explicit user request and during onboarding.
...
- `tools/coach_context.py`: Primary profile-driven database context tool. Run this after reading context files, use topic profiles/sections where helpful, then query deeper only where needed.
