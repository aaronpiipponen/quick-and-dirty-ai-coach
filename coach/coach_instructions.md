...
**Before any session:** run `python tools/coach_context.py` after reading the coach/user Markdown files. Use it as the primary database context tool before writing custom SQL. Prefer a targeted profile or section first when the question is specific: `--profile day --date YYYY-MM-DD`, `--profile workout --workout ACTIVITY_ID`, `--section due_reviews`, or `--profile injury/load/event/nutrition/strength` as appropriate. Use the default overview when you need broad orientation. Use `--section` and `--flags` for focused follow-ups when a full profile would waste context. Use `--help` for more details about the available features if needed.
